...
class LTXImageToVideoPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def prepare_latents(
        ...
        if latents is not None:
            ...
            conditioning_mask[:, :, conditioning_mask.shape[2] // 2] = 1.0
        ...
        if isinstance(generator, list):
            ...
            init_latents = [
                retrieve_latents(self.vae.encode(image.unsqueeze(0).unsqueeze(2)), generator)
                for image in images
            ]
        else:
            init_latents = [
    ...
    def __call__(
        ...
        if latents is None:
            image = [self.video_processor.preprocess(image, height=height, width=width) for image in images]
            image = [image.to(device=device, dtype=prompt_embeds.dtype) for image in images]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print('cond mask', conditioning_mask.shape, 'timestep', timestep, timestep.shape)
                ...
                print('one step done', noise_pred.shape, timestep.shape)
                if self.do_classifier_free_guidance:
