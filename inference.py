...
def load_unet_checkpoint(unet_config_dir, unet_dir):
    unet_ckpt_path = unet_dir / "model.safetensors"
    unet_config_path = unet_config_dir / "config.json"
    transformer_config = Transformer3DModel.load_config(unet_config_path)
    transformer = Transformer3DModel.from_config(transformer_config)
    unet_state_dict = safetensors.torch.load_file(unet_ckpt_path)
    transformer.load_state_dict(unet_state_dict, strict=True)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the directory containing unet checkpoint only",
    )
    parser.add_argument(
        "--input_video_path",
        type=str,
    ...
    parser.add_argument(
        "--start_input_image_path", type=str, help="Path to the start input image file"
    )
    parser.add_argument(
        "--mid_input_image_path", type=str, help="Path to the mid input image file"
    )
    parser.add_argument(
        "--end_input_image_path", type=str, help="Path to the end input image file"
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    # Load image
    if args.start_input_image_path and args.end_input_image_path and args.mid_input_image_path:
        media_items_prepad = [load_image_to_tensor_with_resize_and_crop(
            args.start_input_image_path, args.height, args.width
        ), 
        load_image_to_tensor_with_resize_and_crop(
            args.mid_input_image_path, args.height, args.width
        ),
        load_image_to_tensor_with_resize_and_crop(
            args.end_input_image_path, args.height, args.width
        )]
    else:
        media_items_prepad = None

    height = args.height if args.height else media_items_prepad[0].shape[-2]
    width = args.width if args.width else media_items_prepad[0].shape[-1]
    ...
    if media_items_prepad is not None:
        media_items = [F.pad(
            i, padding, mode="constant", value=-1
        ) for i in media_items_prepad]  # -1 is the value for padding since the image is normalized to -1, 1
    else:
        media_items = None

    # Paths for the separate mode directories
    ckpt_dir = Path(args.ckpt_dir)
    unet_ckpt_dir = Path(args.unet_ckpt_dir)
    ...
    unet = load_unet_checkpoint(unet_dir, unet_ckpt_dir)
    ...
    for i in range(images.shape[0]):
        ...
        if video_np.shape[0] == 1:
            ...
            if args.start_input_image_path:
