...
class LTXVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print('use_interp', use_interpolation)
        ...
        # print("height", height, "width", width, "frames", num_frames)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print("indices grid", indices_grid.shape)
    ...
    def prepare_conditioning(
        ...
        print('media', media_items[0].shape)
        assert media_items[0].ndim == 5
        assert height == media_items[0].shape[-2] and width == media_items[0].shape[-1]

        # Encode the input video and repeat to the required number of frame-tokens
        start_init_latents = vae_encode(
            media_items[0].to(dtype=self.vae.dtype, device=self.vae.device),
            self.vae,
            vae_per_channel_normalize=vae_per_channel_normalize,
        ).float()

        mid_init_latents = vae_encode(
            media_items[1].to(dtype=self.vae.dtype, device=self.vae.device),
            self.vae,
            vae_per_channel_normalize=vae_per_channel_normalize,
        ).float()

        end_init_latents = vae_encode(
            media_items[2].to(dtype=self.vae.dtype, device=self.vae.device),
            self.vae,
            vae_per_channel_normalize=vae_per_channel_normalize,
        ).float()
        ...
        if use_interpolation:
            init_latents = interpolate_frames(init_latents)
...
def interpolate_frames(frames):
    b, c, f, h, w = frames.size()
    f_half = f // 2
    
    # Extract key frames
    frame_0 = frames[:, :, 0, :, :].unsqueeze(2)
    frame_half = frames[:, :, f_half, :, :].unsqueeze(2)
    frame_end = frames[:, :, f-1, :, :].unsqueeze(2)
    
    # Compute interpolation factors
    t1 = torch.arange(f_half + 1, dtype=torch.float32, device=frames.device) / f_half
    t1 = t1.view(1,1,-1,1,1)
    t2 = torch.arange(f - f_half, dtype=torch.float32, device=frames.device) / (f - 1 - f_half)
    t2 = t2.view(1,1,-1,1,1)
    
    # Interpolate frames
    new_frames_first_segment = (1 - t1) * frame_0 + t1 * frame_half
    new_frames_second_segment = (1 - t2) * frame_half + t2 * frame_end
    
    # Combine the two segments
    new_frames = torch.cat([new_frames_first_segment[:, :, :f_half+1, :, :],
                           new_frames_second_segment[:, :, 1:, :, :]], dim=2)
