...
def compute_prompt_embeds(
    ...
    import nvsmi
    # print("Memory before T5",  list(nvsmi.get_gpus())[0])
    text_encoder.to('cuda')
    # print("Memory after T5", list(nvsmi.get_gpus())[0])
    batch_size = text_input_ids.size(0) 
    with torch.no_grad(): 
        prompt_embeds = text_encoder(text_input_ids.to(device))[0]
    text_encoder.to('cpu')
    # print("Memory after T5 cpu", list(nvsmi.get_gpus())[0])
...
def main():
    ...
    unet = LTXVideoTransformer3DModel.from_pretrained('Lightricks/LTX-Video', subfolder='transformer', torch_dtype=load_dtype).to('cuda')
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        vae = AutoencoderKLLTXVideo.from_pretrained('Lightricks/LTX-Video', subfolder='vae', torch_dtype=torch.bfloat16).to('cuda')
    ...
    video_scale_factor, vae_scale_factor = vae.temporal_compression_ratio, vae.spatial_compression_ratio
    step, batch = next(enumerate(train_dataloader))
    ## save batch
    torch.save(batch, 'overfit_batch.pt')
    for epoch in range(first_epoch, args.num_train_epochs): 
        ...
        with accelerator.accumulate(models_to_accumulate): 
            ...
            model_output = compiled_unet(
                hidden_states=noisy_model_input.to(unet.dtype),
...
def debug_video_latents(latents, latent_height, latent_width, latent_num_frames, vae, patchifier,unet, name):
    ...
    out = vae.decode(out.to(vae.dtype))[0]
