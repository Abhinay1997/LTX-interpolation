...
class ImageVideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        while True:
            try:
                video_path = self.video_dir + self.data_list[index]['video']
                ...
                remainder = (7 + (selected_num_frames % 8)) % 8
                if remainder != 0:
                    frames = frames[:-remainder]
                selected_num_frames = frames.shape[0]

                assert (selected_num_frames - 1) % 8 == 0
                if selected_num_frames >= self.min_num_frames:
                    break
                else:
